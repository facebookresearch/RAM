...
def main(input_path = "",
    ...
    tokenizer = AutoTokenizer.from_pretrained("/fsx-ram/shared/Meta-Llama-3.1-8B-Instruct")
    ...
    for d in preference_data:
        if get_sft:
            ...
            prepared_data.append({"chosen": chosen, "rejected": rejected, "prompt": prompt})
        else:
            if no_prompt:
                chosen = d["chosen"]["input"] + d["chosen"]["output"] + tokenizer.eos_token
                rejected = d["rejected"]["input"] + d["rejected"]["output"] + tokenizer.eos_token
                prepared_data.append({"chosen": chosen, "rejected": rejected})
            else:
                chosen = d["chosen"]["output"] + tokenizer.eos_token
                rejected = d["rejected"]["output"] + tokenizer.eos_token
                prepared_data.append({"chosen": chosen, "rejected": rejected, "prompt": d["chosen"]["input"]})
