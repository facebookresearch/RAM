...
def main(saved_path = "/fsx-ram/yifeizhou/collab_llm/outputs/temp_test.jsonl",
    ...
    if not skip_evaluation:
        ...
        for i in tqdm(range(0, len(annotation_results), evaluation_batch_size)):
            with concurrent.futures.ThreadPoolExecutor() as executor:
                jobs = [executor.submit(render_full_html, driver, ground_truth_images[i+j], temp_path, i+j) for j, driver in enumerate(drivers)]
        ...
        for d in drivers:
            d.quit()
        ground_truth_images = [Image.open(ground_truth_image) for ground_truth_image in rendered_images]
        answer_images = [Image.open(answer_image).convert("RGB") if answer_image is not None and os.path.exists(answer_image) else Image.new("RGB", (224, 224), "black") for answer_image in answer_images]
        # import IPython; IPython.embed(); exit()
        model = CLIPModel.from_pretrained("openai/clip-vit-base-patch32").to("cuda")
        processor = CLIPProcessor.from_pretrained("openai/clip-vit-base-patch32")
        
        inputs1 = processor(images=answer_images, return_tensors="pt", padding=True).to("cuda")
        inputs2 = processor(images=ground_truth_images, return_tensors="pt", padding=True).to("cuda")
        ...
        similarities = torch.sum(image_features1 * image_features2, dim=-1).cpu().numpy()
    ...
    print(f"Best-of-{k} Average reward: {np.mean(np.max(raw_correctness_results, axis=0))}")
            ...
            #  if np.max(raw_correctness_results[:, i]) >= 1:
                ...
                for j in range(k):
                    for j2 in range(j+1, k):
                        if abs(raw_correctness_results[j, i] - raw_correctness_results[j2, i]) < 0.1:
                            continue
        ...
        if ground_truth_preference_path is not None:
            with open(ground_truth_preference_path, "w") as fb:
                for preference_pair in preference_pairs:
                    ...
                    chosen = {
                        "input": f"In light that the final answer is: {ground_truth}."+chosen["input"],
                        "output": chosen["output"]
                    }
                    rejected = preference_pair["rejected"]["dialogue_history"][-1]
                    rejected = {
                        "input": f"In light that the final answer is: {ground_truth}."+rejected["input"],
