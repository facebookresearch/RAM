...
class PreprocessedDataset(Dataset):
    ...
    def __getitem__(self, idx):
        if self.split == 'train':
            ...
            x = torch.from_numpy((self.data[idx*self.block_size+random_shift:(idx+1)*self.block_size+random_shift]).astype(np.int64))
        else:
            x = torch.from_numpy((self.data[idx*self.block_size:(idx+1)*self.block_size]).astype(np.int64))
...
def get_train_dataloader(cfg):
    ...
    cfg.n_epochs = cfg.train_steps * cfg.update_batch_size * cfg.grad_acc_steps // len(train_dataset) + 1
