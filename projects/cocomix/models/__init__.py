...
def get_base_lm(cfg, accelerator):
    ...
    if 'gpt2' in cfg.base_model:
        if cfg.n_embd is not None: config.n_embd = cfg.n_embd
        if cfg.n_layer is not None: config.n_layer = cfg.n_layer
        if cfg.n_head is not None: config.n_head = cfg.n_head
        if cfg.mode == 'cocomix': 
            config._attn_implementation = 'flash_attention_2'
            base_lm = GPT2CoCoMixLMHeadModel(config, cfg.concept_dim, cfg.insert_layer_index, cfg.concept_num)
