...
def cosine_scheduler(optimizer, num_warmup_steps, num_training_steps, lr, min_lr, last_epoch=-1):
    def _get_cosine_schedule_with_warmup_lr_lambda(
        current_step: int, *, num_warmup_steps: int, num_training_steps: int, num_cycles: float, lr: float, min_lr: float, 
    ):
        ...
        print (current_step, num_warmup_steps, num_training_steps, num_cycles, lr, min_lr, coeff, progress)
...
def trainer(cfg, train_func, base_lm, train_loader, val_loaders, logger, accelerator, concept_extractor=None):

    main_process = accelerator.is_main_process

    """ get optimizer and scheduler """
    trainable_params = list(base_lm.parameters())
    fused_available = 'fused' in inspect.signature(torch.optim.AdamW).parameters and 'gpt2' in cfg.base_model
    if fused_available: logger.log("Fused AdamW is available")
    
    optimizer = torch.optim.AdamW(trainable_params, lr=cfg.lr, weight_decay=cfg.weight_decay,
                                  betas=(cfg.beta1, cfg.beta2), eps=cfg.eps, fused=fused_available)
    scheduler = get_scheduler(name=cfg.lr_schedule, optimizer=optimizer, num_warmup_steps=cfg.warmup_steps, 
                              num_training_steps=cfg.train_steps, scheduler_specific_kwargs={'min_lr': cfg.min_lr})
    ...
    optimizer, train_loader = accelerator.prepare(
    ...
    for i_epoch in range(0, cfg.n_epochs):
        ...
        for local_step, batch in enumerate(train_loader):
            """ save model """
            if cfg.global_step != 0 and cfg.global_step % cfg.save_step_freq == 0 and local_step % cfg.grad_acc_steps == 0:
            ...
            if cfg.global_step % cfg.eval_step_freq == 0 and local_step % cfg.grad_acc_steps == 0 and val_loaders is not None:
                logger.log(f"-"*50)
                logger.log(f"Start ppl evaluation")
                results = {}
                stime = time.time()
                for val_name, val_loader in val_loaders.items():
                    results[f'{val_name}.ppl'] = evaluate_ppl(cfg, base_lm, val_loader, accelerator, eval_limit=cfg.eval_limit)
            ...
            if cfg.global_step >= cfg.train_steps: break
        if cfg.global_step >= cfg.train_steps: break
    ...
    if main_process:
        torch.save(optimizer.state_dict(), os.path.join(logger.logdir, f'last', 'optimizer.pt'))
        torch.save(scheduler.state_dict(), os.path.join(logger.logdir, f'last', 'scheduler.pt'))
