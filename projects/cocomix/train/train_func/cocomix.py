...
def train_step(cfg, base_lm, optimizer, scheduler, accelerator, batch, logger, metrics_dic, concept_extractor):
    ...
    with accelerator.accumulate(base_lm):
        ...
        outputs, concept_logit = base_lm(input_ids=batch['input_ids'], labels=batch['input_ids'], get_concept_logit=True)
        ...
        for i in range(cfg.topK_attri):
            loss_concept += 1/cfg.topK_attri * F.cross_entropy(concept_logit.view(-1, concept_logit.size(-1)), concept_labels[:, :, i].contiguous().view(-1))
        ...
        if accelerator.sync_gradients:
            ...
            grad_norm = accelerator.clip_grad_norm_(base_lm.parameters(), cfg.grad_clip_thresh)
            ...
            if cfg.global_step % cfg.log_step_freq == 0:
                metric_synchronize_between_processes(metrics_dic, accelerator)  # sync metrics across processes
                log_metrics = {'train': {f'{k}': np.mean(v) for k, v in metrics_dic.items()},
