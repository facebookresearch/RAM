def setup(mode, cfg):
    ...
    if cfg.n_embd is not None: base_model = f"{base_model}_embd{cfg.n_embd}"
    if cfg.n_layer is not None: base_model = f"{base_model}_L{cfg.n_layer}"
    if cfg.n_head is not None: base_model = f"{base_model}_H{cfg.n_head}"

    fname = f"{cfg.dataset}/{base_model}/{mode}" \
            f"_bs{int(cfg.update_batch_size*cfg.grad_acc_steps)}"
    wandb_name = f"{cfg.dataset}_{base_model}_{mode}" \
                 f"_bs{int(cfg.update_batch_size*cfg.grad_acc_steps)}"
