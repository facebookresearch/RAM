    ...
    for batch in val_loader:
        with torch.no_grad():
            if 'labels' not in batch:
                loss = base_lm(input_ids=batch['input_ids'], labels=batch['input_ids']).loss
            else:
...
def evaluate_help(model_hf, tokenizer, batch_size=16, device='cuda', tasks=None, openwebtext_data_dir=None):
    ...
    with torch.no_grad():
        openwebtext_results = evaluate_openwebtext(model_hf, device=device, openwebtext_data_dir=openwebtext_data_dir)
            ...
            for sample in samples:
                if isinstance(sample['target'], int):
                    ...
                    if 'choices' in sample['doc']:
                        target = sample['doc']['choices']['label'].index(sample['target'])
                    else:
...
def evaluate_openwebtext(gpt2_model, device, openwebtext_data_dir):
    ...
    val_data = np.memmap(os.path.join(openwebtext_data_dir, 'val.bin'), dtype=np.uint16, mode='r')
    nll = 0.0
    eff_block_size = 1024
    for i in range(0, len(val_data), eff_block_size):
        if i + eff_block_size >= len(val_data):
            break
        X = torch.from_numpy((val_data[i:i+eff_block_size]).astype(np.int64)).unsqueeze(0).to(device)
        with torch.no_grad():
...
def eval_checkpoint(cfg, checkpoint_path, tokenizer, device):
    ...
    if 'cocomix' in checkpoint_path:
        ...
        if 'gpt2' in checkpoint_path:
            base_lm = GPT2CoCoMixLMHeadModel.from_pretrained(
                checkpoint_path, concept_dim=cfg_load['concept_dim'], insert_layer_idx=cfg_load['insert_layer_index'], concept_num=cfg_load["concept_num"]
            ).to(device)
    else:
        base_lm = AutoModelForCausalLM.from_pretrained(checkpoint_path, **kwargs).to(device)
    ...
    result = evaluate_help(base_lm, tokenizer, cfg.batch_size, device, tasks=tasks, openwebtext_data_dir=cfg.data_dir)
