...
def update_recursively(old_dict, new_dict):
    for key, value in new_dict.items():
        if key in old_dict.keys() and isinstance(value, dict):
            old_dict[key] = update_recursively(old_dict[key], value)
        else:
            old_dict[key] = value
    return old_dict
...
class TrainState(Stateful):
    ...
    def load_state_dict(self, state_dict):
        ...
        # this is a temporary bandaid to fix the problem with checkpoint: it does NOT save TokenizerState from local data, but instead uses old version
        self.data_loader_state = update_recursively(
            self.data_loader_state, new_dict=state_dict["data_loader_state"]
        )
...
def train(args: TrainArgs):
    ...
    with ExitStack() as context_stack:
        ...
        logger.info(f"Vocabulary size: {args.model.vocab_size}")
        ...
        while train_state.step < args.steps:
            ...
            batch, mask, train_state.data_loader_state = next(data_loader)
            batch = torch.tensor(
                batch,
                dtype=torch.long,
            )
            ...
            loss = model(input_ids, labels, loss_mask=mask)
            ...
            loss = (
                loss.detach() * args.grad_acc_steps
            )  # tensor(12.2312, device='cuda:0')
            ...
            if args.eval is not None and every_n_steps(
                ...
                from projects.mta.eval import EVAL_FOLDER_NAME, EvalArgs, launch_eval
                ...
                if args.async_eval_gpus is None:
                    scores = launch_eval(eval_args)
                    metrics.update(flatten_dict(scores))
                    if get_is_master():
                        metric_logger.log(metrics)
                elif get_is_master():
