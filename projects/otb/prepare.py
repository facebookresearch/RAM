...
for row in hmmt_dset['train']:
    new_rows.append({
        'question': row['problem'],
        'answer': row['answer'],
        'metadata': json.dumps({'source': 'hmmt','id': row['problem_idx'], 'problem_type': row['problem_type']}),
        'subset': 'underthinking-math'
    })
...
for row in math_dset['test']:
    new_rows.append({
        'question': row['problem'],
        'answer': row['answer'],
        'metadata': json.dumps({'source': 'math-500','id': row['unique_id'], 'subject': row['subject'], 'level': row['level'], 'solution': row['solution']}),
        'subset': 'overthinking-math'
    })

df =pd.concat([dataset['train'].to_pandas(), pd.DataFrame(new_rows)], ignore_index=True)
df = df[df.apply(lambda x: x['subset']!='overthinkingbench' or 'num_options' not in json.loads(x['metadata']) or json.loads(x['metadata'])['num_options']==4, axis=1)]
