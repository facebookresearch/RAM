...
parser.add_argument("--output", type=str, default="final_outputs/otbench/{{model}}.jsonl")
parser.add_argument("--multiprocessing", type=int, default=1)
parser.add_argument('--enable_nothink', action='store_true', default=False, help='Enable no-think mode for Qwen3 models.')
...
def process_row(row, model_name):
    ...
    response = call_llm(model_name, prompt, max_tokens = 8192 if model_name in ['gpt-4o', 'gpt-4.1'] else 2048 if model_name in ['/datasets/pretrained-llms/Qwen2.5-Math-7B-Instruct', '/datasets/pretrained-llms/Qwen2.5-Math-72B-Instruct'] else 16384, n = args.num_gens, temperature=args.temperature)
    if model_name.find('o3')!=-1:
        # If model returns reasoning tokens, directly use them (if row is of type overthink).
        return ([x.message.content for x in response.choices], response.usage.completion_tokens_details.reasoning_tokens, prompt)
        ...
        return ([x.choices[0].message.content for x in response], sum([x.usage.completion_tokens for x in response]), prompt)
    else:
        # For other models, extract the reasoning tokens during post-processing.
        return ([x.message.content for x in response.choices], response.usage.completion_tokens, prompt)
...
if args.enable_nothink:
    OUTPUT_FILE = OUTPUT.replace('{{model}}', MODEL_NAME.replace('/', '-')).replace('.jsonl', '-nothink.jsonl')
else:
