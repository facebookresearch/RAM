# Principia: Training Language Models to Reason over Mathematical Objects
...
The core claim is simple: if you want models that can help with real scientific and mathematical work, you need to train on such data & test whether they can derive things like equations, sets, matrices, intervals, and piecewise functions.
...
Those formats are convenient to grade, but they hide an important weakness. A model can often solve an MCQ by reasoning backward from the options rather than deriving the answer from first principles.

An example mathematical-object answer from the paper looks like:
...
*Figure: RL training on the Principia Collection improves performance on PrincipiaBench and also transfers to numerical and MCQA benchmarks.*
...
The paper shows that once answer options are removed from SuperGPQA questions that really require mathematical objects, performance drops sharply even for frontier models. The reported drop is typically in the 10 to 20 point range.
...
To train for this harder setting, the paper introduces the **Principia Collection**, a 248K-example synthetic dataset grounded in:
